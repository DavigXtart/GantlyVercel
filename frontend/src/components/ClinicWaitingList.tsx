...
import {
  ListOrdered, Bell, CalendarPlus, Pencil, Trash2, Plus, X, User,
  Mail, Phone, Stethoscope, AlertTriangle, Loader2, Clock, Check,
} from 'lucide-react';
...
export default function ClinicWaitingList() {
  ...
  const handleDelete = async (id: number) => {
    ...
    setActionLoading(id);
    try {
      await clinicService.removeWaitingListEntry(id);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 overflow-hidden">
        ...
        <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] text-slate-400">{entries.length} pacientes</span>
          </div>
          <div className="flex items-center gap-2">
            {/* Status filter */}
            <select
                  ...
                  <tr key={entry.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {entry.psychologistPreference && (
                        <p className="text-[10px] text-slate-400 mt-0.5">Pref: {entry.psychologistPreference}</p>
                      )}
                    </td>
                    {/* Priority */}
                    <td className="px-4 py-3">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-xl border border-slate-200 shadow-lg w-full max-w-md">
            <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={() => setShowAddModal(false)} className="text-slate-400 hover:text-slate-600 transition-colors cursor-pointer bg-transparent border-none p-1">
                <X size={16} />
              </button>
            </div>
      ...
      {editEntry && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40" onClick={() => setEditEntry(null)} />
          <div className="relative bg-white rounded-xl border border-slate-200 shadow-lg w-full max-w-sm">
            <div className="px-5 py-3 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={() => setEditEntry(null)} className="text-slate-400 hover:text-slate-600 transition-colors cursor-pointer bg-transparent border-none p-1">
                <X size={16} />
              </button>
            </div>
            <div className="p-5 space-y-3">
